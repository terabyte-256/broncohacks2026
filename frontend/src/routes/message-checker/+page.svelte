...
<Card title="Message input" description="Paste a message to analyze for phishing indicators.">
<form
class="space-y-3"
onsubmit={(event) => {
...
required
/>
<Button type="submit" loading={$analysisState.status === 'loading'}>Analyze message</Button>
</form>
</Card>
...
<Badge variant={riskVariant($analysisState.data.riskLevel)}>{$analysisState.data.riskLevel}</Badge>
<p class="text-sm text-text-muted">{$analysisState.data.verdict}</p>
</div>
<p class="text-sm text-text">{$analysisState.data.explanation}</p>
<div>
<p class="text-xs font-semibold uppercase tracking-wide text-text-muted">Red flags</p>
{#if $analysisState.data.redFlags.length}
<ul class="mt-2 space-y-1 text-sm">
{#each $analysisState.data.redFlags as flag, index (`${flag}-${index}`)}
<li class="rounded-md border border-border bg-muted px-2.5 py-1.5">{flag}</li>
{/each}
</ul>
{:else}
