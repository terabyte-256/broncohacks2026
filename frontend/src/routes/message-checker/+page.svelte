<script lang="ts">
  ...
  function riskVariant(
    level: MessageAnalysis["riskLevel"],
...
>
  {#snippet actions()}
    <Button variant="secondary" onclick={() => analysisState.reset()}
      >Reset</Button
    >
  {/snippet}
</SectionHeader>
          ...
          {#if $analysisState.data.redFlags.length}
            <ul class="mt-2 space-y-1 text-sm">
              {#each $analysisState.data.redFlags as flag, index (`${flag}-${index}`)}
                <li
                  class="rounded-md border border-border bg-muted px-2.5 py-1.5"
                >
                  {flag}
                </li>
              {/each}
